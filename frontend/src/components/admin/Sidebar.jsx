...
export default function Sidebar({ institution, user, open, onClose }) {
  ...
  const navItems = isPlatformAdmin
    ? [{ key: 'applications', icon: Building2, page: 'InstitutionApplications' }]
    : isStaff
      ? [...baseNav]
      : [...baseNav, ...adminNav];
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-medium">{navLabels[item.key] || t(item.key)}</span>
                </Link>
              );
            })}
          </nav>
